...
const CALLBACK_URL = `${BASE_URL}/github/callback`

/**
 * Returns the redirect url for the twitter request
 */
async function getRedirectUrl(): Promise<string> {
	const redirectRequest = new UrlBuilder(github.REDIRECT_URL)
...
async function getAccessToken(code: string) {
	const requestToken = new Oauth2Request(github.ACCESS_TOKEN_URL, {
		clientId: CLIENT_ID,
...
createServer(async (req, res) => {
	try {
		if (req.url === '/') {
			...
			res.end(`<a href="${await getRedirectUrl()}">Login with github</a>`)
		} else {
