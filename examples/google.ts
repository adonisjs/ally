...
const CALLBACK_URL = `${BASE_URL}/google/callback`

/**
 * Returns the redirect url for the twitter request
 */
async function getRedirectUrl(): Promise<string> {
	const redirectRequest = new UrlBuilder(google.REDIRECT_URL)
	...
	redirectRequest.param('access_type', 'offline')
	redirectRequest.param('prompt', 'select_account')
	redirectRequest.param('scope', 'email')
...
async function getAccessToken(code: string) {
	const requestToken = new Oauth2Request(google.ACCESS_TOKEN_URL, {
		clientId: CLIENT_ID,
...
createServer(async (req, res) => {
	try {
		if (req.url === '/') {
			...
			res.end(`<a href="${await getRedirectUrl()}">Login with google</a>`)
		} else {
